...
function ContactMe({ userEmail, userPhone, userGithub, userLinkedin, userInsta, userFacebook, userX, userYoutube }) {
    return (
        <div
            data-aos="zoom-in-up"
        ...
        >
            ...
            <div className="flex lg:flex-row flex-col w-[80%] mx-auto gap-4 lg:items-start items-center justify-center lg:justify-between">
                <div className="flex flex-col gap-1 lg:gap-4 w-full lg:max-w-[30vw]">
                    ...
                    <a
                        className="lg:text-2xl text-gray-200 underline hover:text-pink-400 hover:drop-shadow-[0_0_6px_#ff4ddb]  duration-200 "
                        href={`mailto:${userEmail}`}
                    >
                        {`${userEmail || ""}`}
                    </a>
                    <a
                        className="lg:text-2xl lg:mb-0 mb-2 text-gray-200 underline hover:text-cyan-400 hover:drop-shadow-[0_0_6px_#3ecfff] transition duration-200 cursor-pointer"
                        href={`tel:+91${userPhone}`}
                    >
                        {`+91 ${userPhone || ""}`}
                    </a>
                    <div className="wrapper ">
                        <a href={`${userGithub}`} target="_blank" className="button">
                            <div className="icons">
                                <FaGithub className="icon ph:size-7" />
                            </div>
                            <span>Github</span>
                        </a>
                        <a href={`${userLinkedin}`} target="_blank" className="button">
                            <div className="icons">
                                <FaLinkedin className="icon" />
                            </div>
                            <span>Linkedin</span>
                        </a>
                        <a href={`${userInsta}`} target="_blank" className="button">
                            <div className="icons">
                                <FaInstagramSquare className="icon" />
                            </div>
                            <span>Instagram</span>
                        </a>
                        <a href={`${userFacebook}`} target="_blank" className="button">
                            <div className="icons">
                                <FaFacebook className="icon" />
                            </div>
                            <span>Facebook</span>
                        </a>
                        <a href={`${userX}`} target="_blank" className="button">
                            <div className="icons">
                                <FaSquareXTwitter className="icon" />
                            </div>
                            <span>X</span>
                        </a>
                        <a href={`${userYoutube}`} target="_blank" className="button">
                            <div className="icons">
                ...
                >
                    ...
                    />
                    <textarea
                        className="bg-transparent border-2 lg:px-2 lg:mb-5 lg:py-3 rounded-lg placeholder-[#7e8286] h-[200px] lg:h-[250px] outline-none focus:border-purple-500 text-gray-200 resize-none focus:pl-5 lg:focus:pl-6 transition-all duration-100 lg:text-xl font-mono text-base px-2 py-1 caret-gray-200 mb-3 "
