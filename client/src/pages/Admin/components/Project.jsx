...
function Project({ project, handleDelete }) {
    ...
    const addProject = async (projectId) => {
        ...
        let res;
        try {
            res = await axios.post(`http://localhost:3000/api/updateProject/${projectId}`, {
                name,
    ...
    return (
        <div>
            {
                edit ? (<div className="flex flex-col h-[50vh] select-none w-[300px] overflow-hidden border-opacity-60 hover:shadow-lg transition-all delay-100 duration-300 rounded-xl p-2 gap-1 border-2 border-gray-400 backdrop-blur-xl">
                    <div className="flex flex-col">
                        <p className=" gradient-text lg:text-base">Name</p>
                        <input placeholder='portfoli side' required value={name} onChange={(e) => setname(e.target.value)} className=' bg-transparent border-2   px-2 py-[2px]  rounded-lg text-gray-200 outline-none focus:border-purple-500 focus:pl-4 transition-all duration-100 lg:text-base text-sm font-mono mb-1  lg:mb-3' type="text" />
                    </div>
                    <div className="flex flex-col ">
                    ...
                    <div className="flex flex-col">
                        ...
                        <input placeholder='https://github.com' required value={githubLink} onChange={(e) => setGithubLink(e.target.value)} className=' bg-transparent border-2   px-2 py-[2px]  rounded-lg text-gray-200 outline-none focus:border-purple-500 focus:pl-4 transition-all duration-100 lg:text-base text-sm font-mono mb-1  lg:mb-3' type="text" />
                    </div>
                    <div className="flex flex-col ">
                        <p className=" gradient-text lg:text-base">Live Url</p>
                        <input placeholder='https://live.example.com' required value={liveLink} onChange={(e) => setLiveLink(e.target.value)} className=' bg-transparent border-2   px-2 py-[2px]  rounded-lg text-gray-200 outline-none focus:border-purple-500 focus:pl-4 transition-all duration-100 lg:text-base text-sm font-mono mb-1  lg:mb-3' type="text" />
                    </div>

                </div>) : (<div className="h-[40vh] select-none w-[250px] flex flex-col overflow-hidden border-opacity-60 hover:shadow-lg transition-all delay-100 duration-300 rounded-xl border-2 border-gray-400 backdrop-blur-xl">
                    {/* picture of the project */}
                    <div>
                        <img
                            src={image}
                            alt="project over view pic"
                            className="lg:h-[150px]  lg:w-[300px]"
                        />
                    </div>

                    {/* details section  */}
                    <div className="gap-2 p-2 flex flex-col">
                        <h3 className=" text-2xl text-justify text-gray-200">
                            {name}
                        </h3>
                        <p className="text-sm min-h-8 text-gray-400">
                            {description}
                        </p>
            ...
            <div className="flex w-[15vw] mt-3 rounded-md py-[4px] px-6 items-center justify-between">
                <MdDelete onClick={() => { handleDeleteProject(project._id) }} className="text-white lg:text-2xl size-6 cursor-pointer hover:text-amber-500" />
                {
                    edit ? <MdOutlineDownloadDone onClick={() => addProject(project._id)} className="text-white lg:text-2xl size-6 cursor-pointer hover:text-amber-500" /> :
                        <MdEdit onClick={() => setEdit(!edit)} className="text-white lg:text-2xl size-6 cursor-pointer hover:text-amber-500" />
                }
            </div>
        </div>
    )
